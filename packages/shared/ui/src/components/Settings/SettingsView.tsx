...
interface ThemePreviewCardProps {
  theme: ThemeConfig;
  isActive: boolean;
  isPreferred?: boolean; // Show as preferred for auto mode
  onSelect: () => void;
  onDelete?: () => void;
  onEdit?: () => void;
}
...
const ThemePreviewCard: React.FC<ThemePreviewCardProps> = ({
  ...
  return (
    <button
      className={`theme-preview-card ${isActive ? 'active' : ''} ${isPreferred ? 'preferred' : ''}`}
      onClick={onSelect}
    ...
    >
      ...
      >
        {/* Mode badge */}
        <div className={`theme-preview-mode-badge ${theme.mode}`}>
          {theme.mode === 'dark' ? <MoonIcon size={10} /> : <SunIcon size={10} />}
        </div>
        {/* Mini sidebar */}
        <div
      ...
      <div className="settings-content">
        ...
        <section className="settings-section">
          ...
          <div className="settings-option">
            <div className="settings-option-info">
              <h3>Follow System</h3>
              <p>Automatically switch between your preferred dark and light themes based on system settings</p>
            </div>
            <button
              className={`settings-toggle ${autoMode ? 'active' : ''}`}
          ...
          {autoMode && (
            <div className="settings-auto-info">
              <div className="settings-auto-info-item">
                <MoonIcon size={14} />
                <span>Dark: {themes.find(t => t.id === preferredDarkThemeId)?.name || 'Audiio Dark'}</span>
              </div>
              <div className="settings-auto-info-item">
                <SunIcon size={14} />
                <span>Light: {themes.find(t => t.id === preferredLightThemeId)?.name || 'Audiio Light'}</span>
              </div>
            </div>
          )}
          ...
          <div className="settings-theme-group">
            ...
            <p className="settings-theme-hint">
              {autoMode
                ? 'Select a theme to set it as your preferred theme for that mode'
            ...
            <div className="settings-theme-grid">
              {builtinThemes.map((theme) => (
                <ThemePreviewCard
                  ...
                  isPreferred={autoMode && (theme.id === preferredDarkThemeId || theme.id === preferredLightThemeId)}
        ...
        {activeTab === 'audio' && (
          <section className="settings-section">
            ...
            <p className="settings-section-description">
              Configure audio processing, crossfade, and other playback settings.
            </p>
            <AudioSettings />
          </section>
        )}
        ...
        {activeTab === 'connection' && (
          <section className="settings-section">
            ...
            <p className="settings-section-description">
              Manage your connection to the Audiio server. View connection status and switch servers.
            </p>
            <ConnectionSettings />
          </section>
        )}
