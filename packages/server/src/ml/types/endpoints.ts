...
export interface FeatureEndpoint {
  ...
  /** Extract features for a track (alias for get) */
  extract(trackId: string): Promise<AggregatedFeatures>;
...
export interface UserEndpoint {
  ...
  updatePreferences(preferences: Partial<UserPreferencesUpdate>): Promise<void>;
}
...
export interface UserPreferences {
  ...
  /** Artist preferences map (artistId -> affinity -100 to 100) */
  artistPreferences?: Record<string, number>;

  /** Genre preferences map (genre -> affinity -100 to 100) */
  genrePreferences?: Record<string, number>;

  /** Hourly energy patterns */
  hourlyEnergy?: number[];

  /** Hourly genre preferences */
  hourlyGenres?: Record<number, string[]>;

  /** User exploration level (0-1) */
  explorationLevel?: number;

  /** Diversity weight for recommendations (0-1) */
  diversityWeight?: number;
}
