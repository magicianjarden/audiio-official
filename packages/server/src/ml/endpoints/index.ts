...
export function createEndpoints(deps: EndpointDependencies): MLCoreEndpoints {
  ...
  // Event listeners
  const eventListeners = new Set<(event: UserEvent) => void>();
  ...
  const user: UserEndpoint = {
    ...
    getStats: async () => {
      const events = eventRecorder.getListenEvents();
      const uniqueTracks = new Set(events.map(e => e.track.id)).size;
      const uniqueArtists = new Set(events.map(e => e.track.artistId).filter(Boolean)).size;
      const totalListenTime = events.reduce((sum, e) => sum + (e.duration || 0), 0);

      // Calculate top hour/day from temporal patterns
      const patterns = await preferenceStore.getTemporalPatterns();
      const topHour = patterns.hourlyFrequency.indexOf(Math.max(...patterns.hourlyFrequency));
      const topDay = patterns.dailyFrequency.indexOf(Math.max(...patterns.dailyFrequency));
      ...
      return {
        ...
        uniqueArtists,
        totalListenTime,
        averageSessionLength: events.length > 0 ? totalListenTime / events.length : 0,
        topHour,
        topDay,
      };
    },
    updatePreferences: async (preferences) => {
      if (preferences.explorationLevel !== undefined) {
        preferenceStore.setExplorationLevel?.(preferences.explorationLevel);
      }
      if (preferences.diversityWeight !== undefined) {
        preferenceStore.setDiversityWeight?.(preferences.diversityWeight);
      }
    },
  };
  ...
  const scoring: ScoringEndpoint = {
    ...
    getOtherAlgorithmScores: async (trackId) => {
      const results: AlgorithmScoreEntry[] = [];
  ...
  const events: EventEndpoint = {
    ...
    on: (type, callback) => {
      const wrappedCallback = (event: UserEvent) => {
        if (event.type === type) {
