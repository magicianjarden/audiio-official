...
export class PreferenceStore extends StorageHelper {
   ...
   */
  private recordListen(event: ListenEvent): void {
   ...
   * Record a skip event
   */
  private recordSkip(event: SkipEvent): void {
   ...
   * Record a like event
   */
  private recordLike(event: LikeEvent): void {
  ...
  /**
   * Set exploration level (0-1)
   * Higher values = more new/unfamiliar tracks
   */
  setExplorationLevel(level: number): void {
    this.explorationLevel = Math.max(0, Math.min(1, level));
    this.save();
  }
  ...
  /**
   * Set diversity weight (0-1)
   * Higher values = more variety in recommendations
   */
  setDiversityWeight(weight: number): void {
    this.diversityWeight = Math.max(0, Math.min(1, weight));
    this.save();
  }
   ...
   */
  getDiversityWeight(): number {
    return this.diversityWeight;
  }
